...
rem
echo. >>%_InstallFile%
echo rem [%DATE% %TIME%] Java   : Temurin OpenJDK %Java20% >>%_InstallFile%
echo rem >>%_InstallFile%
rem Remove previous stuff
rd /S /Q %_InstallPath%\java\%Java20% 2>NUL
md %_InstallPath%\java\%Java20% >NUL 2>&1
pushd %_InstallPath%\java\%Java20%
rem Uncompress
...
popd
echo rem Set Temurin OpenJDK 20 Environment Variables >>%_InstallFile%
echo rem set "JDK_20_HOME=%_InstallPath%\java\%Java20%\jdk" >>%_InstallFile%
echo rem set "JRE_20_HOME=%_InstallPath%\java\%Java20%\jre" >>%_InstallFile%
echo. >>%_InstallFile%
echo rem Set Temurin OpenJDK 20 System Environment Variables >>%_InstallFile%
echo setx JDK_20_HOME %_InstallPath%\java\%Java20%\jdk /M >>%_InstallFile%
echo setx JRE_20_HOME %_InstallPath%\java\%Java20%\jre /M >>%_InstallFile%
echo rem >>%_InstallFile%
echo rem setx JAVA_HOME %%%%JDK_20_HOME%%%% /M >>%_InstallFile%
echo rem setx JRE_HOME %%%%JRE_20_HOME%%%% /M >>%_InstallFile%
echo. >>%_InstallFile%
rem
echo.
echo Finished.
